...
Vagrant.configure(VAGRANTFILE_API_VERSION) do |config|
  ...
  config.vm.define "samplemix" do |company|
	company.vm.box = "samplemix"
	# Configure some Virtual Box params
	company.vm.provider :virtualbox do |company|
		company.customize ["modifyvm", :id, "--name", "SampleMix"]
	...
	company.vm.box_url = "https://cloud-images.ubuntu.com/vagrant/trusty/current/trusty-server-cloudimg-amd64-vagrant-disk1.box"
        ...
        if host_os =~ /linux/ or host_os =~ /darwin/
			company.vm.synced_folder "/var/www/samplemix", "/var/www/samplemix", type: "nfs", version: 3, nfs_udp: 1, mount_options: ["rw", "tcp", "nolock", "noacl", "sync"]
        else
			company.vm.synced_folder "/var/www/samplemix", "/var/www/samplemix", owner: "www-data", group: "www-data"
        end
